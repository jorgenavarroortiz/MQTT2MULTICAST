...
class tcp(packet_base.PacketBase):
    ...
    def __init__(self, src_port=1, dst_port=1, seq=0, ack=0, offset=0,
        ...
        LOG.debug("######### TCP INIT") # JNa: debugging for MQTT
...
# JNa: for MQTT
        elif src_port == mqtt.TCP_SERVER_PORT or dst_port == mqtt.TCP_SERVER_PORT:
#            LOG.debug("######### TCP - get_payload_type: MQTT")
            return mqtt.mqtt
# End JNa
        else:
            return None
    ...
    def parser(cls, buf):
        ...
        LOG.debug("######### TCP PARSER") # JNa: debugging for MQTT
        offset >>= 4
        bits &= 0x3f
        length = offset * 4
    ...
    def serialize(self, payload, prev):
        LOG.debug("######### TCP SERIALIZE") # JNa: debugging for MQTT
